...
function quickSort(array) {//[2,13]
  if (array.length <= 1) {
    return array;
  }

  // get random pivot element (and remove from array to add back in later)
  var pivot = array.splice(Math.floor(Math.random() * array.length), 1);//2

  // create left array (elements <= pivot), and right array (elements > pivot)
  var left = [];//[]
  var right = [];//[13]

  // loop through array and create left/right// [13]
...
//           V                     V  
// quickSort([]).concat(2, quickSort([13])
